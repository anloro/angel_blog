...
### SO(3)
We shall then introduce **SO(3)**. The group of orthonormal matrices with determinant 1 is called the **Special Orthonormal Group** of ℝ<sup>3</sup> or **SO(3)**, and can be defined in the following manner:
...
Given (R, dR) 𝜖 TSO(3), dR 𝜖 T<sub>R</sub>SO(3), T a tangent plane to SO(3), it satisfies the conditions:
...
- [x] **Lie bracket operation [,]**: *[w̃<sub>x</sub>, w̃<sub>y</sub>] := w̃<sub>x</sub>w̃<sub>y</sub> - w̃<sub>y</sub>w̃<sub>x</sub> 𝜖 so(3)*

### so(3)
We can also define the algebra of this group as **so(3)**, which it is a vector space tangent to the identity element of SO(3). We have the following theorem:
...
- [x] It has the Lie bracket operation:
...
Now define the tilde operator as:
